...
@app.route('/bikesinfo', methods=['GET'])
def get_bike():
    c, conn = connection()
    c.execute(''' SELECT * FROM bikeinfo ''')
    rows = c.fetchall()
    conn.close()
    # print(rows)
    bike_data = []
...
def webhook_handler():
    ...
    if event_type == "entry.create":
        if not check_table_exists(table_name):
            create_table_query = "CREATE TABLE IF NOT EXISTS {} (id INT PRIMARY KEY)".format(table_name)     # If the table doesn'not exist then create it 
            c, conn = connection()
